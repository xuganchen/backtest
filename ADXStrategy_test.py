...
def run(config, trading_data, window):
    ...
    strategy = ADXStrategy(data_handler, events_queue, suggested_quantity = 100,
                           window = window)
...
def run_1min():
    ...
    config_in = {
        ...
        "out_dir": "F:/Python/backtest/backtest/results/ADXStrategy/1min/in_sample",
        "title": "ADXStrategy",
        "is_plot": False,
        "save_plot": False,
        "save_tradelog": False,
    ...
    for i in range(len(interval)):
        window = interval[i]
        config_in['title'] = "ADXStrategy" + "_" + str(window)
        print("---------------------------------")
        print(config_in['title'])
        print("---------------------------------")
        backtest, results = run(config_in, trading_data, window = window)
        dict_ans = {
            "window": window,
    ...
    ans.to_csv("F:/Python/backtest/backtest/results/ADXStrategy/1min"  + "/result_ADXStrategy_in_sample.csv")
    ...
    for i in range(len(best_window)):
        window = best_window.iloc[i]
        config_out['title'] = "ADXStrategy" + "_" + str(window)
        print("---------------------------------")
        print(config_out['title'])
        print("---------------------------------")
        backtest, results = run(config_out, trading_data, window = window)
        dict_ans = {
            "window": window,
            "Sharpe Ratio": results['sharpe'],
            "Total Returns": (results['cum_returns'][-1] - 1),
    ...
    best.to_csv("F:/Python/backtest/backtest/results/ADXStrategy/1min"  + "/result_ADXStrategy_out_sample.csv")
...
def run_60min():
    ...
    if not os.path.exists(config_in['out_dir']):
        os.makedirs(config_in['out_dir'])
    ans.to_csv("F:/Python/backtest/backtest/results/ADXStrategy/60min"  + "/result_ADXStrategy_in_sample.csv")
    ans = ans.sort_values(by = "Total Returns", ascending=False)
    ...
    if not os.path.exists(config_out['out_dir']):
        os.makedirs(config_out['out_dir'])
    best.to_csv("F:/Python/backtest/backtest/results/ADXStrategy/60min"  + "/result_ADXStrategy_out_sample.csv")
