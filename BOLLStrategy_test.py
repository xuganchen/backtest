...
def run(config, trading_data, window = 10, a = 2):
    ...
    strategy = BOLLStrategy(data_handler, events_queue, suggested_quantity=100,
                            window = window, a = a)
...
def run_1min():
    ...
    config_in = {
        ...
        "out_dir": "F:/Python/backtest/backtest/results/BOLLStrategy/1min/in_sample",
        "title": "BOLLStrategy",
        "is_plot": False,
        "save_plot": False,
        "save_tradelog": False,
    ...
    for i in range(len(window_interval)):
        for j in range(len(a_interval)):
            ...
            a = a_interval[j]
            config_in['title'] = "BOLLStrategy" + "_" + str(window) + "_" + str(a)
            print("---------------------------------")
            print(config_in['title'])
            print("---------------------------------")
            backtest, results = run(config_in, trading_data, window=window, a=a)
            dict_ans = {
                "window": window,
                "a": a,
    ...
    ans.to_csv("F:/Python/backtest/backtest/results/BOLLStrategy/1min" + "/result_BOLLStrategy_in_sample.csv")
    ...
    config_out = {
        "csv_dir": "F:/Python/backtest/trades_Bitfinex_folder",
        "out_dir": "F:/Python/backtest/backtest/results/BOLLStrategy/1min/out_sample",
        "title": "BOLLStrategy",
        "is_plot": True,
        "save_plot": True,
        "save_tradelog": True,
    ...
    for i in range(len(best_window)):
        window = best_window.iloc[i]
        a = best_a.iloc[i]
        config_out['title'] = "BOLLStrategy" + "_" + str(window) + "_" + str(a)
        print("---------------------------------")
        print(config_out['title'])
        print("---------------------------------")
        backtest, results = run(config_out, trading_data, window = window, a = a)
        dict_ans = {
            "window": window,
            "a": a,
            "Sharpe Ratio": results['sharpe'],
    ...
    best.to_csv("F:/Python/backtest/backtest/results/BOLLStrategy/1min" + "/result_BOLLStrategy_out_sample.csv")
...
def run_60min():
    ...
    config_in = {
        "csv_dir": "F:/Python/backtest/trades_Bitfinex_folder",
        "out_dir": "F:/Python/backtest/backtest/results/BOLLStrategy/60min/in_sample",
        "title": "BOLLStrategy",
        "is_plot": False,
        "save_plot": False,
        "save_tradelog": False,
    ...
    if not os.path.exists(config_in['out_dir']):
        os.makedirs(config_in['out_dir'])
    ans.to_csv("F:/Python/backtest/backtest/results/BOLLStrategy/60min" + "/result_BOLLStrategy_in_sample.csv")
    ans = ans.sort_values(by="Total Returns", ascending=False)
    ...
    config_out = {
        "csv_dir": "F:/Python/backtest/trades_Bitfinex_folder",
        "out_dir": "F:/Python/backtest/backtest/results/BOLLStrategy/60min/out_sample",
        "title": "BOLLStrategy",
        "is_plot": True,
        "save_plot": True,
        "save_tradelog": True,
    ...
    if not os.path.exists(config_out['out_dir']):
        os.makedirs(config_out['out_dir'])
    best.to_csv("F:/Python/backtest/backtest/results/BOLLStrategy/60min" + "/result_BOLLStrategy_out_sample.csv")
