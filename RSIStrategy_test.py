...
def run(config, trading_data, ohlc_data,  window = 10, s=70, b=30):
    config['title'] = "RSIStrategy" + "_" + str(window) + "_" + str(s) + "_" + str(b)
    print("---------------------------------")
    print(config['title'])
    print("---------------------------------")
    backtest, results = run_backtest(config, trading_data, ohlc_data, window=window, s=s, b=b)
    dict_ans = {
        "window": window,
        "s": s,
        "b": b,
...
## 样本内
    ...
    config = {
        ...
        "out_dir": "C:/backtest/results/RSIStrategy/in_sample",
        "title": "RSIStrategy",
        "is_plot": False,
        "save_plot": False,
        "save_tradelog": False,
    ...
    ans.to_csv(config['out_dir'] + "/result_RSIStrategy_in_sample.csv")
...
## 样本外
    config = {
        "csv_dir": "C:/backtest/Binance",
        "out_dir": "C:/backtest/results/RSIStrategy/out_sample",
        "title": "RSIStrategy",
        "is_plot": True,
        "save_plot": True,
        "save_tradelog": True,
    ...
    if not os.path.exists(config['out_dir']):
        os.makedirs(config['out_dir'])
    ans = ans.sort_values(by="Total Returns", ascending=False)
    ans.to_csv(config['out_dir'] + "/result_RSIStrategy_out_sample.csv")