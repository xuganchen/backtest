...
def acq_max(ac, gp, y_max, bounds, is_int, random_state, n_warmup=100000, n_iter=250):
    """
    ...
    for col, (lower, upper) in enumerate(bounds):
        if is_int[col]:
            x_tries.T[col] = random_state.randint(lower, upper, size=n_warmup)
        else:
            x_tries.T[col] = random_state.uniform(lower, upper, size=n_warmup)
    ...
    for col, (lower, upper) in enumerate(bounds):
        if is_int[col]:
            x_seeds.T[col] = random_state.randint(lower, upper, size=n_iter)
        else:
            x_seeds.T[col] = random_state.uniform(lower, upper, size=n_iter)
    ...
    for x_try in x_seeds:
        ...
        for i in range(bounds.shape[0]):
            if is_int[i]:
                res_x[i] = int(res_x[i])
        res_fun0 = ac(res_x.reshape(1, -1), gp=gp, y_max=y_max)
        # Store it if better than previous minimum(maximum).
        if max_acq is None or res_fun0 >= max_acq:
            x_max = res_x
            max_acq = -res_fun0
