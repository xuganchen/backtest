    ...
    def generate_signals(self, event):
        if event.type == EventType.MARKET:
            ...
            if bars is not None and bars != []:
                ...
                md = np.sum(np.abs(tp - tp_ma))/t
                cci = (tp_current - tp_ma) / (0.015 * md)

                if cci > self.threshold and self.holdinds[ticker] == "EMPTY":
                    self.generate_buy_signals(ticker, bar_date, "LONG")
                    self.holdinds[ticker] = "HOLD"
                elif cci < -self.threshold and self.holdinds[ticker] == "HOLD":
...
if __name__ == "__main__":
    ...
    backtest, results = run_backtest(config, trading_data, ohlc_data, window = 119)
