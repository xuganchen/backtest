...
def run_backtest(config, trading_data, ohlc_data, window_BOLL, a, window_EMV, n, m):
...
def running(X):
    config = {
        ...
        "out_dir": os.path.join(back_config['output_folder'], "BOLL_EMVStrategy"),
    ...
    a = X[0,1]
    window_EMV = int(X[0,2])
    n = int(X[0,3])
    m = int(X[0,4])
    return run_backtest(config, trading_data, ohlc_data, window_BOLL, a, window_EMV, n, m)
