    ...
    def generate_signals(self, event):
        if event.type == EventType.MARKET:
            ...
            high = self.data_handler.get_latest_bars_values(
                ticker, "high", N=self.window
            )
            low = self.data_handler.get_latest_bars_values(
                ticker, "low", N=self.window
            )
            volume = self.data_handler.get_latest_bars_values(
                ticker, "volume", N=self.window
            )
            bar_date = event.timestamp
            
            if bars is not None and bars != []:
                close = bars
                shift_close = np.roll(bars, 1)
                shift_close = shift_close[1:]
                close = close[1:]
                volume = volume[1:]
                fi = (close - shift_close)*volume
                mafi = np.mean(fi)
                
                if mafi > 0 and self.holdinds[ticker] == "EMPTY":
                    self.generate_buy_signals(ticker, bar_date, "LONG")
                    self.holdinds[ticker] = "HOLD"
                elif mafi < 0 and self.holdinds[ticker] == "HOLD":
...
def run_backtest(config, trading_data, ohlc_data, window = 25):
    config['title'] = "CMFStrategy" + "_" + str(window)
    print("---------------------------------")
    print(config['title'])
    print("---------------------------------")
    ...
    strategy = CMFStrategy(config, events_queue, data_handler,
                            window=window)
...
if __name__ == "__main__":
    ...
    backtest, results = run_backtest(config, trading_data, ohlc_data, window = 25)
