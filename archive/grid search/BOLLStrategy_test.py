...
def run_backtest(config, trading_data, ohlc_data, window = 10, a = 2):
...
def run(config, trading_data, ohlc_data, window, a):
    config['title'] = "BOLLStrategy" + "_" + str(window) + "_" + str(a)
    print("---------------------------------")
    print(config['title'])
    print("---------------------------------")
    backtest, results = run_backtest(config, trading_data, ohlc_data, window=window, a=a)
    dict_ans = {
        "window": [window],
        "a": [a],
...
## 样本内
    ...
    config = {
        ...
        "out_dir": "C:/backtest/results/BOLLStrategy/in_sample",
        "title": "BOLLStrategy",
        "is_plot": False,
        "save_plot": False,
        "save_tradelog": False,
    ...
    ans.to_csv(config['out_dir'] + "/result_BOLLStrategy_in_sample.csv")
...
## 样本外
    config = {
        "csv_dir": "C:/backtest/Binance",
        "out_dir": "C:/backtest/results/BOLLStrategy/out_sample",
        "title": "BOLLStrategy",
        "is_plot": True,
        "save_plot": True,
        "save_tradelog": True,
    ...
    if not os.path.exists(config['out_dir']):
        os.makedirs(config['out_dir'])
    ans = ans.sort_values(by="Total Returns", ascending=False)
    ans.to_csv(config['out_dir'] + "/result_BOLLStrategy_out_sample.csv")