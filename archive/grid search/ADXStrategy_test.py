...
def run(config, trading_data, ohlc_data, window):
    config['title'] = "ADXStrategy" + "_" + str(window)
    print("---------------------------------")
    print(config['title'])
    print("---------------------------------")
    backtest, results = run_backtest(config, trading_data, ohlc_data, window=window)
    dict_ans = {
        "window": window,
...
## 样本内
    ...
    config = {
        ...
        "out_dir": "C:/backtest/results/ADXStrategy/in_sample",
        "title": "ADXStrategy",
        "is_plot": False,
        "save_plot": False,
        "save_tradelog": False,
    ...
    for i in range(len(interval)):
        window = interval[i]
        result = pool.apply_async(run, args=(config, trading_data, ohlc_data, window,))
    ...
    ans.to_csv(config['out_dir'] + "/result_ADXStrategy_in_sample.csv")
...
## 样本外
    config = {
        "csv_dir": "C:/backtest/Binance",
        "out_dir": "C:/backtest/results/ADXStrategy/out_sample",
        "title": "ADXStrategy",
        "is_plot": True,
        "save_plot": True,
        "save_tradelog": True,
    ...
    if not os.path.exists(config['out_dir']):
        os.makedirs(config['out_dir'])
    ans = ans.sort_values(by="Total Returns", ascending=False)
    ans.to_csv(config['out_dir'] + "/result_ADXStrategy_out_sample.csv")